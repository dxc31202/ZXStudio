...
public class SysImageList : IDisposable
{
    ...
    [DllImport("comctl32")]
    extern static int ImageList_Draw(
            IntPtr hIml,
            int i,
            IntPtr hdcDst,
            int x,
    ...
    [DllImport("comctl32")]
    extern static int ImageList_DrawIndirect(
            ref IMAGELISTDRAWPARAMS pimldp);

    [DllImport("comctl32")]
    extern static int ImageList_GetIconSize(
            IntPtr himl,
            ref int cx,
            ref int cy);
    ...
    [DllImport("shell32.dll", EntryPoint = "#727")]
    extern static int SHGetImageListHandle(
            int iImageList,
            ref Guid riid,
            ref IntPtr handle
            );
    ...
    #region ImageList COM Interop (XP)
    [ComImportAttribute()]
    ...
    public SysImageListSize size = SysImageListSize.smallIcons;
    bool disposed = false;
    #endregion
    ...
    public Icon Icon(int index)
    {
        ...
        int flags = (int)ImageListDrawItemConstants.ILD_TRANSPARENT
            | (int)ImageListDrawItemConstants.ILD_PRESERVEALPHA | (int)ImageListDrawItemConstants.ILD_SCALE;
        IntPtr hIcon = IntPtr.Zero;
        if (iImageList == null)
            hIcon = ImageList_GetIcon(hIml, index, (int)ImageListDrawItemConstants.ILD_TRANSPARENT);
        else {
            iImageList.GetIcon(index, flags, ref hIcon);
        }
        //bool x= ThumbnailCache.NativeMethods.ImageList_SetOverlayImage(Handle, index, 1);
    ...
    public int IconIndex(IntPtr PIDL)
    {
        return IconIndex(PIDL, false);
    }
    ...
    /// <returns>Index of the icon</returns>
    public int IconIndex(IntPtr PIDL, bool forceLoadFromDisk)
    {
        return IconIndex(PIDL, forceLoadFromDisk, ShellIconStateConstants.ShellIconStateNormal);
    }
    ...
    /// returned.</param>
    /// <returns>Index of the icon</returns>
    public int IconIndex(IntPtr PIDL, bool forceLoadFromDisk, ShellIconStateConstants iconState)
    {
        ...
        IntPtr retVal = SHGetFileInfo(PIDL, dwAttr, ref shfi, shfiSize, ((uint)(dwFlags) | (uint)iconState));
    ...
    {
        if (iImageList == null)
            ImageList_Draw(hIml, index, hdc, x, y, (int)flags);
        else
        {
    ...
    public virtual void Dispose(bool disposing)
    {
        if (!disposed)
        {
            if (disposing)
            {
                if (iImageList != null) Marshal.ReleaseComObject(iImageList);
                iImageList = null;
            }
        }
        disposed = true;
    ...
    ~SysImageList()
    {
        // System.Diagnostics.Debug.Write(this.GetType().Name + ":");
        Dispose(false);
    }
...
public class SysImageListHelper
{
    ...
    public delegate void SetListViewImageListDelegate(ListView listView, SysImageList sysImageList, bool forStateImages);
    public delegate void SetTreeViewImageListDelegate(TreeView treeView, SysImageList sysImageList, bool forStateImages);
    /// <summary>
    ...
    public static void SetListViewImageList(ListView listView, SysImageList sysImageList, bool forStateImages)
    {
        ...
        if (sysImageList.ImageListSize == SysImageListSize.smallIcons) wParam = (IntPtr)LVSIL_SMALL;
        if (forStateImages) wParam = (IntPtr)LVSIL_STATE;
    ...
    public static void SetTreeViewImageList(TreeView treeView, SysImageList sysImageList, bool forStateImages)
    {
        ...
        if (forStateImages) wParam = (IntPtr)TVSIL_STATE;
